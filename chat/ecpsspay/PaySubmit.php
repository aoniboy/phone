...
<?
	  ...
	  if ($defaultBankNumber=="NOCARD"){
	     $MD5key = "mxuJxBfQ";		//MD5私钥
         $MerNo = "26208";					//商户号
		  $ReturnURL = "http://".$_SERVER['HTTP_HOST']."/chat/ecpsspay/PayResult_n.php"; 
		  $AdviceURL ="http://".$_SERVER['HTTP_HOST']."/chat/ecpsspay/PayResult_n.php";  
	  }else
	  {
	   $MD5key = "mxuJxBfQ";		//MD5私钥
       $MerNo = "26208";					//商户号
	   $ReturnURL = "http://".$_SERVER['HTTP_HOST']."/chat/ecpsspay/PayResult.php"; 
       $AdviceURL ="http://".$_SERVER['HTTP_HOST']."/chat/ecpsspay/PayResult.php";  
	  }
	  
    
     $BillNo =$_REQUEST["order_no"]."_".date('YmdHis');		//[必填]订单号(商户自己产生：要求不重复)
     $Amount = $_REQUEST["price"];		//[必填]订单金额
    ...
    $products=$_REQUEST["order_no"];// '------------------物品信息
...
<form name="py" action="http://ycshenbang.cn/chat/ecpsspay/Payclint.php" method="post"  >
  <table align="center">
