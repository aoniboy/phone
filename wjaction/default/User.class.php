...
class User extends WebBase {
    ...
    public final function login() {
        $this->display('newuser/login.php');
    }
    ...
    public final function loginedto() {
        ...
        if (!ctype_alnum($username))
            throw new Exception('用户名包含非法字符,请重新登陆');
    ...
    public final function registered() {
        ...
        if (!ctype_alnum($user))
            $this->outputData(1, array(), '用户名包含非法字符');
        if (!ctype_digit($qq))
            $this->outputData(1, array(), 'QQ包含非法字符');
        ...
        if (!$_POST['lid'])
            $para['lid'] = $lid;
        if (!$linkData)
            $this->outputData(1, array(), '不存在此注册链接。');
        if (!$parentId)
            $this->outputData(1, array(), '链接错误');
        $para = array(
            'username' => $user,
        ...
        try {
            ...
            if ($this->getValue($sql, $para['username']))
                $this->outputData(1, array(), '用户"' . $para['username'] . '"已经存在');
