...
class Team extends WebLoginBase{
	...
	public final function insertLink(){
		...
		if($update['fanDianBdw']<0) $this->outputData(1,array(),'不定位返点不能小于0');
		...
		if($update['uid']!=$this->user['uid']) $this->outputData(1,array(),'只能增加自己的推广链接!');

		// 查检返点设置
		if($update['fanDian']){
			$this->getSystemSettings();
			if($update['fanDian'] % $this->settings['fanDianDiff']) $this->outputData(1,array(),sprintf('返点只能是%.1f%的倍数', $this->settings['fanDianDiff']));
		...
		try{
			...
			if($this->getValue($sql, $update['fanDian'])) $this->outputData(1,array(),'此链接已经存在');
	...
	public final function linkUpdateed(){
        ...
        if($uid=$this->getvalue("select uid from {$this->prename}links where lid=?",$lid)){
		     if($uid!=$this->user['uid']) $this->outputData(1,array(),'只能修改自己的推广链接!');
		}else{
...
//		if($qqlen<4 || $qqlen>13) $this->outputData(1,array(),'QQ号为4-12位,请重新输入');
		...
		if($update['fanDian']){
			...
			if($count && $count['registerCount']>=$count['userCount']) $this->outputData(1,array(),sprintf('对不起返点为%.1f的下级人数已经达到上限', $update['fanDian']));
		}else{
			$update['fanDian']=0.0;
		}
 ...
 /*额度转移*/
	public final function userUpdateed2(){
		...
		if(!$para=$_POST) $this->outputData(1,array(),'提交数据出错，请重新操作');

        if($this->settings['recharge']==1){
		$uid=intval($para['uid']);
		$uid2=$this->user['uid'];
		$para['coin']=floatval($para['coin']);
		if($para['coin']<1 || $para['coin']>10000) $this->outputData(1,array(),'只能充值1-10000元');
        ...
        if(!$userData2=$this->getRow($sql, $uid)) $this->outputData(1,array(),'此会员不存在!');

		if($userData2['parentId']!=$uid2) $this->outputData(1,array(),'此会员不是的你的直属会员，请重新选择！');
		if(!$userData2['enable']) $this->outputData(1,array(),'此会员已被冻结，无法转移！');
		if($userData['coin']<=0) $this->outputData(1,array(),'余额不足，请先充值！');
		if($userData['coin']<$para['coin']) $this->outputData(1,array(),'可用余额小于充值金额，请先充值！');
	  ...
	  }else{ $this->outputData(1,array(),'上级充值功能已经关闭！');}
	}
}