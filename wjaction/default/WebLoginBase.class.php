...
class WebLoginBase extends WebBase {
    ...
    public function addCoin($log) {
        ...
        if (!isset($log['extfield2']))
            $log['extfield2'] = '';
        if (!isset($log['isDelete']))
            $log['isDelete'] = 0;
        $sql = "call setCoin({$log['coin']}, {$log['fcoin']}, {$log['uid']}, {$log['liqType']}, {$log['type']}, '{$log['info']}', {$log['extfield0']}, '{$log['extfield1']}', '{$log['extfield2']}',0)";
