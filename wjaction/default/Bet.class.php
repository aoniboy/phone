...
class Bet {
    ...
    public static function ssc5xfs($bet) {
        ...
        foreach ($bets as $b) {
            ...
            if (count($codes) != count($x) || count($codes) > 10 || count($codes) < 1)
                return 0;
            foreach ($codes as $y) {
                if (!in_array($y, $check))
                    return 0;
    ...
    public static function sscqh2zhixhz($bet) {
        ...
        $alist = $bet;
        $a = count($alist);
        if (count($bet) < 1 || count($bet) > 18 || count($bet) != count($x) || $e != count($bet) - 1)
            return 0;
        foreach ($bet as $b) {
            if (!in_array($b, $check))
                return 0;
    ...
    public static function rx2fs($bet) {
        ...
        foreach ($bets as $b) {
            ...
            if (count($codes) != count($count) || count($codes) > 10 || count($codes) < 1)
                return 0;
            foreach ($codes as $codess) {
                if (!in_array($codess, $check))
                    return 0;
            }
            $ret *= count($codes);
        }
    ...
    public static function r2ssc($bet) {
        ...
        $k = $bet;
        $bet = explode(' ', $bet);
        $bet1 = array_unique($bet);
        if (count($bet) != count($bet1) || count($bet) < 2 || count($bet) > 10)
            return 0;
        foreach ($bet as $bets) {
            if (!in_array($bets, $check))
                return 0;
        }
        return self::rx($k, 2);
    }
    ...
    public static function qh2dxds($bet) {
        ...
        $a1 = array_unique($a);
        $b1 = array_unique($b);
        if (count($a) < 1 || count($b) < 1 || count($a) > 4 || count($b) > 4)
            return 0;
        if (count($a) != count($a1) || count($b) != count($b1))
            return 0;
        return self::fs($k);
    }
    ...
    public static function qh3dxds($bet) {
        ...
        $b1 = array_unique($b);
        $c1 = array_unique($c);
        if (count($a) < 1 || count($b) < 1 || count($c) < 1 || count($a) > 4 || count($b) > 4 || count($c) > 4)
            return 0;
        if (count($a) != count($a1) || count($b) != count($b1) || count($c) != count($c1))
            return 0;
        return self::fs($k);
    }
    ...
    public static function rxdxds($bet) {
        ...
        foreach ($bets as $s) {
            $a = str_split($s);
            $a1 = array_unique($a);
            if (count($a) < 1 || count($a) > 4 || count($a) != count($a1))
                return 0;
            $ret *= count($a);
        }
        return $ret;
    ...
    public static function zx120($bet) {
        ...
        if (count($bet) != count($bet0) || count($bet) > 10 || count($bet) < 5)
            return 0;
        foreach ($bet as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function zx60($bet) {
        ...
        $dCode = str_split($bet[0]);
        $tCode = str_split($bet[1]);
        $a1 = array_unique($dCode);
        $b1 = array_unique($tCode);
        $dLen = count($dCode);
        $tLen = count($tCode);
        if ($dLen < 1 || $dLen > 10 || $dLen != count($a1))
            return 0;
        if ($tLen < 3 || $tLen > 10 || $tLen != count($b1))
            return 0;
        foreach ($dCode as $x) {
            if (!in_array($x, $check))
                return 0;
    ...
    public static function zx30($bet) {
        ...
        $dLen = count($dCode);
        $tLen = count($tCode);
        if ($dLen < 2 || $dLen > 10 || $dLen != count($a1))
            return 0;
        if ($tLen < 1 || $tLen > 10 || $tLen != count($b1))
            return 0;
        foreach ($dCode as $x) {
            if (!in_array($x, $check))
                return 0;
    ...
    public static function zx20($bet) {
        ...
        if ($dLen < 1 || $dLen > 10 || $dLen != count($a1))
            return 0;
        if ($tLen < 2 || $tLen > 10 || $tLen != count($b1))
            return 0;
        foreach ($dCode as $x) {
            if (!in_array($x, $check))
                return 0;
    ...
    public static function zx24($bet) {
        ...
        $bet = explode(',', $bet);
        $dLen = count($bet);
        $repet = array_unique($bet);
        $endnum = 0;
        if ($dLen < 4 || $dLen > 10 || count($repet) != $dLen)
            return 0;
        foreach ($bet as $x) {
            if (!in_array($x, $check))
                return 0;
    ...
    public static function zx12($bet) {
        ...
        if ($num - 1 >= 0) {
            ...
            if ($dLen - $num > 0) {
                ...
                $anum = $anum + $sele_count[$tLen - 1] * ($dLen - $num);
            }
        } else {
            if ($tLen - 1 >= 0) {
                $c = $tLen - 1;
    ...
    public static function zx6($bet) {
        ...
        $repet = array_unique($bet);
        $dLen = count($bet);
        if ($dLen > 10 || $dLen < 2 || count($repet) != $dLen)
            return 0;
        foreach ($bet as $x) {
            if (!in_array($x, $check))
                return 0;
    ...
    public static function ssch3zhixkd($bet) {
        ...
        $dLen = count($bet);
        $endnum = 0;
        if ($dLen < 1 || $dLen > 10 || count($repet) != $dLen)
            return 0;
        foreach ($bet as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function ssch3tshm($bet) {
        ...
        $bet = explode(',', $bet);
        $bet1 = array_unique($bet);
        if (count($bet) < 1 || count($bet) > 3 || count($bet) != count($bet1))
            return 0;
        return count($bet);
    }
    ...
    public static function sscqzhr31m($bet) {
        ...
        $bet = str_split($bet);
        $bet1 = array_unique($bet);
        if (count($bet) < 1 || count($bet) > 10 || count($bet) != count($bet1))
            return 0;
        foreach ($bet as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function ssc5x3m($bet) {
        ...
        $bet = str_split($bet);
        $bet1 = array_unique($bet);
        if (count($bet) < 3 || count($bet) > 10 || count($bet) != count($bet1))
            return 0;
        foreach ($bet as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r1sscqw($bet) {
        ...
        $bet = explode(' ', $bet);
        $bet0 = array_unique($bet);
        if (count($bet) != count($bet0) || count($bet) > 10 || count($bet) < 1)
            return 0;
        foreach ($bet as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function qh3qw2x($bet) {
        ...
        $b1 = count($b);
        $c1 = count($c);
        $repet = count(array_unique($a));
        $repet1 = count(array_unique($b));
        $repet2 = count(array_unique($c));
        if ($a1 != $repet || $b1 != $repet1 || $c1 != $repet2)
            return 0;
        if ($a1 < 1 || $b1 < 1 || $c1 < 1 || $a1 > 2 || $b1 > 10 || $c1 > 10)
            return 0;
        foreach ($bet as $s) {
            $k = str_split($s);
            foreach ($k as $h) {
    ...
    public static function ssc4mqw3x($bet) {
        ...
        $repet1 = count(array_unique($b));
        $repet2 = count(array_unique($c));
        $repet3 = count(array_unique($d));
        if ($a1 != $repet || $b1 != $repet1 || $c1 != $repet2 || $d1 != $repet3)
            return 0;
        if ($a1 < 1 || $b1 < 1 || $c1 < 1 || $d1 < 1 || $a1 > 2 || $b1 > 10 || $c1 > 10 || $d1 > 10)
            return 0;
        foreach ($bet as $s) {
            $k = str_split($s);
            foreach ($k as $h) {
    ...
    public static function ssc5mqw3x($bet) {
        ...
        $repet2 = count(array_unique($c));
        $repet3 = count(array_unique($d));
        $repet4 = count(array_unique($e));
        if ($a1 != $repet || $b1 != $repet1 || $c1 != $repet2 || $d1 != $repet3 || $e1 != $repet4)
            return 0;
        if ($a1 < 1 || $b1 < 1 || $c1 < 1 || $d1 < 1 || $e1 < 1 || $a1 > 2 || $b1 > 2 || $c1 > 10 || $d1 > 10 || $e1 > 10)
            return 0;
        foreach ($bet as $s) {
            $k = str_split($s);
            foreach ($k as $h) {
    ...
    public static function r111x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 1)
            return 0;
    ...
    public static function r211x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 2)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r311x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 3)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r411x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 4)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r511x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 5)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r611x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 6)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r711x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 7)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function r811x5($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 11 || count($bet1) < 8)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function q1zx11x5($bet) {
        ...
        $a = explode(' ', $bet);
        $c = array_unique($a);
        if (count($a) != count($c) || count($a) > 11 || count($a) < 1)
            return 0;
        foreach ($a as $b) {
            if (!in_array($b, $check))
                return 0;
    ...
    public static function dwd11x5($bet) {
        ...
        foreach ($a as $b) {
            $g = explode(' ', $b);
            $c = array_unique($g);
            if (count($g) != count($c) || count($g) > 11 || count($g) < 1)
                return 0;
        }
        $x = str_replace(array(',', '-', ' '), '', $bet);
        $z = str_split($x, 2);
    ...
    public static function bdw11x5($bet) {
        ...
        if (count($bet) != count($count) || count($bet) > 11 || count($bet) < 1)
            return 0;
        foreach ($bet as $s) {
            if (!in_array($s, $check))
                return 0;
    ...
    public static function k3hz($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 16 || count($bet1) < 1)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function k33tdx($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 6 || count($bet1) < 1)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function k33ttx($bet) {
        ...
        $bet1 = explode(',', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) != 6)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function k33bt($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) < 3 || count($bet1) > 6)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function k32bt($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) < 2 || count($bet1) > 6)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function k32tdx($bet) {
        ...
        $bet[0] = explode(' ', $bet[0]);
        $bet[1] = explode(' ', $bet[1]);
        $a = array_unique($bet[0]);
        $b = array_unique($bet[1]);
        if (count($bet[0]) != count($a) || count($bet[1]) != count($b) || count($bet[0]) > 6 || count($bet[0]) < 1 || count($bet[1]) > 6 || count($bet[1]) < 1)
            return 0;
        foreach ($bet[0] as $x) {
            if (!in_array($x, $check))
                return 0;
    ...
    public static function pk10cgyj($bet) {
        ...
        foreach ($bets as $b) {
            ...
            if (count($codes) != count($m) || count($codes) > 11 || count($codes) < 1)
                return 0;
            foreach ($codes as $k) {
                if (!in_array($k, $check))
                    return 0;
    ...
    public static function lhctm($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 49 || count($bet1) < 1)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function lhc2z2($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 49 || count($bet1) < 2)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function lhc3z3($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) > 49 || count($bet1) < 3)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function lhc5bz($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) != 5)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function lhc7bz($bet) {
        ...
        $bet1 = explode(' ', $bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) != 7)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function fc3dhz($bet) {
        ...
        $a = array_unique($bet1);
        $endnum = 0;
        if (count($bet1) != count($a) || count($bet1) < 1 || count($bet1) > 28)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function fc3dzxhz($bet) {
        ...
        $a = array_unique($bet1);
        $endnum = 0;
        if (count($bet1) != count($a) || count($bet1) < 1 || count($bet1) > 27)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
    ...
    public static function fc3d1mbdw($bet) {
        ...
        $bet1 = str_split($bet);
        $a = array_unique($bet1);
        if (count($bet1) != count($a) || count($bet1) < 1 || count($bet1) > 10)
            return 0;
        foreach ($bet1 as $bets) {
            if (!in_array($bets, $check))
                return 0;
