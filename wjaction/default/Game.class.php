...
class Game extends WebLoginBase {
    ...
    public final function postCode() {
        ...
        if ($urlcheck != $urldan)
            $this->outputData(1, array(), '请勿站外投注，谢谢合作');

        $codes = $_POST['code'];
        $para = $_POST['para'];
        $amount = 0;
        $fpcount = 1;  //飞盘 默认为1
        if (!ctype_digit($codes[0]['actionNum']))
            $this->outputData(1, array(), '注数只能为整数');
        if (!ctype_digit($codes[0]['beiShu']))
            $this->outputData(1, array(), '倍数只能为整数');
        if (!ctype_digit($codes[0]['weiShu']))
            $this->outputData(1, array(), '位数只能为整数');
        $this->getSystemSettings();
        if ($this->settings['switchBuy'] == 0)
            $this->outputData(1, array(), '本平台已经停止购买！');
        if ($this->settings['switchDLBuy'] == 0 && $this->user['type'])
            $this->outputData(1, array(), '代理不能买单！');
        if (count($codes) == 0)
            $this->outputData(1, array(), '请先选择号码再提交投注');
        //检查时间 期数
        ...
        if ($actionTime != $para['kjTime'])
            $this->outputData(1, array(), '投注失败：你投注第' . $para['actionNo'] . '已过购买时间');
        ...
        foreach ($codes as $code) {
            ...
            //检查开启
            if (!$played['enable'])
                $this->outputData(1, array(), '游戏玩法组已停,请刷新再投');
            //检查赔率
            ...
            if (floatval($code['fanDian']) > floatval($this->user['fanDian']) || floatval($code['fanDian']) > floatval($this->settings['fanDianMax']))
                $this->outputData(1, array(), '提交返点出错，请重新投注');
            //检查倍数
            if (intval($code['beiShu']) < 1)
                $this->outputData(1, array(), '倍数只能为大于1正整数');
            //检查模式
            ...
            if ($code['actionNum'] > $maxcount)
                $this->outputData(1, array(), '注数超过该玩法最高注数:' . $maxcount . '注,请重新投注');

            $code = current($codes);
            if (!isset($para['qzEnable']))
                $para['qzEnable'] = 0;
            if ($para['fpEnable'])
                $fpcount = 2;
            ...
            if ($zhuihao = $_POST['zhuiHao']) {
                ...
                foreach (explode(';', $zhuihao) as $var) {
                    ...
                    if (strtotime($actionNo['actionTime']) - $ftime < $this->time)
                        $this->outputData(1, array(), '投注失败：你追号投注第' . $code['actionNo'] . '已过购买时间');
                    $codes[] = $code;
        ...
        if ($userAmount < $amount)
            $this->outputData(1, array(), '您的可用资金不足，请充值。');
    ...
    public final function znzPost($id) {
        ...
        if ($para['fanDianAmount'] < 0)
            throw new Exception('参数错误3');
        if ($para['qz_chouShui'] < 0)
        ...
        try {
            ...
            if ($para['qz_fcoin'] < $amount)
                throw new Exception('参数错误6');
            if ($data['isDelete'])
                throw new Exception('投注已经撤单');
            if ($data['qz_uid'])
                throw new Exception('已经被别人抢庄了');
            if ($data['uid'] == $this->user['uid'])
                throw new Exception('不能抢自己的庄');
            if ($amount > $this->user['coin'])
                throw new Exception('你的资金余额不足');

            // 冻结时间后不能抢庄
            $this->getTypes();
            $ftime = $this->getTypeFtime($data['type']);
            if ($data['kjTime'] - $ftime < $this->time)
                throw new Exception('这期已经结冻，不能抢庄');
    ...
    public final function deleteCode($id) {
        ...
        try {
            ...
            if (!$data = $this->getRow($sql, $id))
                $this->outputData(1, array(), '找不到定单。');
            if ($data['isDelete'])
                $this->outputData(1, array(), '这单子已经撤单过了。');
            if ($data['uid'] != $this->user['uid'])
                $this->outputData(1, array(), '这单子不是您的，您不能撤单。');  // 可考虑管理员能给用户撤单情况
            if ($data['kjTime'] <= $this->time)
                $this->outputData(1, array(), '已经开奖，不能撤单');
            if ($data['lotteryNo'])
                $this->outputData(1, array(), '已经开奖，不能撤单');
            if ($data['qz_uid'])
                $this->outputData(1, array(), '单子已经被人抢庄，不能撤单');

            // 冻结时间后不能撤单
            $this->getTypes();
            $ftime = $this->getTypeFtime($data['type']);
            if ($data['kjTime'] - $ftime < $this->time)
                $this->outputData(1, array(), '这期已经结冻，不能撤单');
    ...
    public final function post28Code() {
        $urlshang = $_SERVER['HTTP_REFERER']; //上一页URL
        $urldan = $_SERVER['HTTP_X_REAL_HOST']; //本站域名
        $urlcheck = substr($urlshang, 7, strlen($urldan));
        if ($urlcheck != $urldan)
