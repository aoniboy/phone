...
function freshKaiJiangData(type){
	$('#historylot').load('/index.php/index/getHistoryData/'+type);
}
...
function teamBeforeAddMember(){
	...
	if(!this.fanDianBdw.value){winjinAlert("请输入不定位返点","alert");return false;}
...
function tzCombineSelect2(){
	var codeLen=parseInt(this.attr('length')),
	codes='', $select=$('.checked'),len,fangan=$('#positioninfo').html();
	if($select.length<codeLen) throw('请选'+codeLen+'位数');
	$select.each(function(){
		codes+=this.value;
	});
...
function tzSscHhzxInput(){
	...
	z3=[],
	z6=[];
	var o={"前":[16,17],"后":[19,20],"任选":[22,23],"混":[59,60]};
...
function ssczx60(){
	var code=[], len=1,codeLen=parseInt(this.attr('length'));
	var endnum=0;var num=0;var c;var anum=0;var bnum=0;var c;var d;
...
function ssczx20(){
	...
	var $d=$(this).filter(':visible:first'),
		...
		//}else if(tLen<2){
			...
			for (i=0;i<tLen-1;i++){d=i+1;for (j=d;j<tLen;j++){for (c=0;c<dLen;c++){if(dCode[i]-tCode[c]!=0 && dCode[j]-tCode[c]!=0){bnum=bnum+1;}}}}
			len=bnum;
			return {actionData:dCode.join('')+','+tCode.join(''), actionNum:len};
		}
}
